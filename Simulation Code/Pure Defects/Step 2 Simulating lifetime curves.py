...
def calculate_tau_effective(delta_n, T, tau_p0_1, Et_1, k_1):
...
def calculate_all_tau_effs(tau_p0_1, Et_1, k_1):
    all_tau_effs=[]
    for T in Temperatures:
        tau_eff_full_curve = []
        for delta_n in range(0, len(Injections)):
            delta_n_value = Injections[delta_n]
            tau_eff = calculate_tau_effective(delta_n_value, T, tau_p0_1, Et_1, k_1)
            tau_eff_full_curve.append(tau_eff)
        all_tau_effs.append(tau_eff_full_curve)
    return all_tau_effs


def simulate_lifetime(all_tau_effs, tau_p0_1, Et_1, k_1, title, directory):
    ...
    plt.legend([leg1], ['$k_1$='+str(k_1)+', $E_{t,1}$='+str(Et_1)], handlelength=0, loc='upper center')
    ...
    for T in Temperatures:
        ...
        for i in Delta_n_values:
            tau_eff = calculate_tau_effective(i, T, tau_p0_1, Et_1, k_1)
...
for data in tau_p0_data:
    ...
    tau_p0_1 = data['tau_p0_1_a']
    Et_1 = data['Et_1']
    k_1 = data['k_1']
    title = data['impurity']
    directory = data['directory']
    all_tau_effs = calculate_all_tau_effs(tau_p0_1, Et_1, k_1)
    simulate_lifetime(all_tau_effs, tau_p0_1, Et_1, k_1, title, directory)