...
fractions=[0.5, 0.625, 0.75, 0.875]
...
def calculate_tau_effective(delta_n, T, tau_p0_1, tau_p0_2): #Equations to calculate tau_SRH:
...
def calculate_for_fractions():
    ...
    for fraction in fractions:
        ...
        for tau_p0_1_value in tau_p0_1:
            tau_p0_2 = fraction*tau_p0_1_value/(1-fraction)
            result = calculate_tau_effective(Injections[53], Temperatures[0], tau_p0_1_value, tau_p0_2) #Injection[53] equals 5e14 cm-3
            lifetime_at_5e14.append(result)
            meta_lifetime_at_5e14.append((tau_p0_1_value, tau_p0_2, result))
...
for impurity in impurities:
    Et_1 = impurity["Et"]
    k_1 = impurity["k"]
    for other in impurities:
        if other["name"] != impurity["name"]:  # Do not compare a defect with itself
            # comparison = set([other['name'],impurity['name']])
            # if comparison in comparisons:
            #     continue  # Disregard this comparison if it already has been ran before
            # comparisons.append(comparison)
            Et_2 = other["Et"]
            k_2 = other["k"]
            result = calculate_for_fractions()
            results.append(
                {
                    "impurity"      : impurity["name"],
                    "Et_1"          : impurity["Et"],
                    "k_1"           : impurity["k"],
                    "combined_with" : other["name"],
                    "Et_2"          : other["Et"],
                    "k_2"           : other["k"],
                    "tau_p0_1_a"    : result[0],
                    "tau_p0_2_a"    : result[1],
...
for result in results:
    ...
    subdirectory = result["combined_with"]
    path = './tau_p0_data/'+ directory + '/'+ subdirectory
    try:
        os.makedirs(path)
    except OSError:
        pass
